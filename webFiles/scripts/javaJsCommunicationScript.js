...
function checkInputForError (input) {
    ...
    if (input == socketCreateErr || input == writeSocketErr
            || input == readSocketErr) {
...
function getGrammarsAsJSONObject () {
    /* request grammar as json string and parse to get the grammars object */
    allGrammarsAsJSON = JSON.parse (writeStringToJava (requestDelim+getAllGrammarsRequest));
    //return the object with all the grammars 
    return allGrammarsAsJSON;
}
